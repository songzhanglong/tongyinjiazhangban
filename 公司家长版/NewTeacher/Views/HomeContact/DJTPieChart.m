...
- (void)setShowPercentage:(BOOL)showPercentage
{
    ...
    for(SliceLayer *layer in _pieView.layer.sublayers)
    {
        ...
        NSDictionary *attribute = @{NSFontAttributeName: self.labelFont};
        CGSize size = [label sizeWithAttributes:attribute];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (point.x < 15 || point.x > _pieView.frameRight - 15 || point.y < 15 || point.y > _pieView.frameBottom - 15) {
        return;
    }
...
- (SliceLayer *)createSliceLayer
{
    ...
    NSDictionary *attribute = @{NSFontAttributeName: self.labelFont};
    CGSize size = [@"0" sizeWithAttributes:attribute];
    [CATransaction setDisableActions:YES];
...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value
{
    ...
    if (value > 0) {
        ...
        {
            //[textLayer setString:@""];
            [textLayer setString:[NSString stringWithFormat:@"%d", countNum]];
            [textLayer setBounds:CGRectMake(0, 0, size.width+5, size.height)];
        }
        else
        {
            //[textLayer setString:label];
